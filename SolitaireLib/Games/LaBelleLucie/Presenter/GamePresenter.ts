...
export class GamePresenter extends GamePresenterBase<IGame> {
    ...
    private layoutPiles_() {
        ...
        const yTop = vExpand * -45 * scale + scaledMargin;
        ...
        const row1Y = yTop + this.sizeY + 1.5 * scale;
        for (let i = 0; i < 6; ++i) {
            const pile = this.game_.tableaux[i] ?? error();
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(i), row1Y);
            pileView.fanYDown = 1.2 * scale;
            pileView.fanYUp = 1.2 * scale;
        }

        // Tableau row 2: tableaux 6 to 11
        const row2Y = row1Y + this.sizeY + 3.5 * scale;
        for (let i = 6; i < 12; ++i) {
            const pile = this.game_.tableaux[i] ?? error();
            const pileView = this.getPileView_(pile);
        ...
        // Tableau row 3: tableaux 12 to 17
        const row3Y = row2Y + this.sizeY + 3.5 * scale;
        for (let i = 12; i < 18; ++i) {
            const pile = this.game_.tableaux[i] ?? error();
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(i - 12), row3Y);
            pileView.fanYDown = 1.2 * scale;
            pileView.fanYUp = 1.2 * scale;
        }
    }
